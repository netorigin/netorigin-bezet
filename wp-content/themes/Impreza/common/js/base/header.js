			...
			if ( this.pos == 'fixed' ) {
				if ( this.orientation == 'hor' ) {
					if ( ( $us.canvas.headerInitialPos == 'bottom' || $us.canvas.headerInitialPos == 'below' ) && ( $us.$body.usMod( 'state' ) == 'default' ) ) {
						...
						if ( ! this.$container.hasClass( 'sticky' ) && _scrollTop >= scrollBreakpoint ) {
							this.$container.addClass( 'sticky' );
						} else if ( this.$container.hasClass( 'sticky' ) && ! scrollTop && _scrollTop < scrollBreakpoint ) {
							this.$container.removeClass( 'sticky' );
						}
						if ( !! this.pid ) {
						...
						this.pid = setTimeout( function() {
							if ( parseInt( $us.$window.scrollTop() ) === 0 && this.$container.hasClass( 'sticky' ) ) {
								this.$container.removeClass( 'sticky' );
							}
							clearTimeout( this.pid );
						}.bind( this ), 1 );
